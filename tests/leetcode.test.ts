 ...
 *
 * Tests one model at a time against 10 LeetCode problems.
 * Each problem response is saved to outputs/<model>/leetcode/<slug>/
 *
 * Run:
 *   MODEL=qwen7b   npx jest tests/leetcode.test.ts --runInBand
 *   MODEL=deepseek npx jest tests/leetcode.test.ts --runInBand
 *   MODEL=claude   npx jest tests/leetcode.test.ts --runInBand
 *   MODEL=litellm  npx jest tests/leetcode.test.ts --runInBand
 *   MODEL=langchain npx jest tests/leetcode.test.ts --runInBand
 */
...
import { writeSuiteSummary, OutputRecord } from "../src/utils/output-writer";
...
describe(`LeetCode Evaluation — ${client.name} (${client.modelId})`, () => {
  beforeAll(async () => {
    ...
    if (!available) {
      throw new Error(`Model ${client.modelId} is not available.`);
    }
  ...
  afterAll(() => {
    ...
    const avgLatency  = allResults.reduce((s, r) => s + r.latencyMs, 0) / (allResults.length || 1);
    ...
    log.info(`Overall: ${totalPassed}/${totalTests} passed`);
    log.info(`Avg latency: ${avgLatency.toFixed(0)}ms  |  Total cost: $${totalCost.toFixed(4)}`);

    const records: OutputRecord[] = allResults.map((r) => ({
      model: r.model,
      suite: "leetcode",
      slug: r.problem.slug,
      rawResponse: r.rawResponse,
      extractedCode: r.generatedCode,
    ...
    writeSuiteSummary(client.modelId, "leetcode", records);
  });
  ...
  describe("Easy", () => {
    test("LC#1 — Two Sum", async () => {
      ...
      expect(result.passed).toBeGreaterThanOrEqual(Math.floor(result.total * 0.75));
    });
  ...
  describe("Medium", () => {
    test("LC#3 — Longest Substring Without Repeating Characters", async () => {
      ...
      allResults.push(result);
      logTestResults(result);
      expect(result.passed).toBeGreaterThanOrEqual(Math.floor(result.total * 0.5));
    });
