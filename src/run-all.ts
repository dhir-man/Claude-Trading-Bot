/**
 * Run LeetCode evaluation across all available models and print a comparison table.
...
import { writeSuiteSummary, OutputRecord } from "./utils/output-writer";
...
async function main() {
  ...
  for (const key of MODELS) {
    ...
    for (const problem of PROBLEMS) {
      ...
      log.info(`  ${icon} ${problem.title.padEnd(48)} ${r.passed}/${r.total} (${r.latencyMs}ms)`);
    }

    writeSuiteSummary(client.modelId, "leetcode", outputRecords);
