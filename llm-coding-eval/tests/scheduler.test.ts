/**
 * Scheduler/Reminder App Evaluation Suite
 *
 * Each model is asked to generate a complete ReminderService class.
...
describe(`Scheduler App — ${MODEL_KEY}`, () => {
  beforeAll(async () => {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert TypeScript engineer. Respond ONLY with a ```typescript code block.",
        },
    ...
    log.dim("--- Generated Code (first 500 chars) ---");
    log.dim(code.slice(0, 500));
    log.dim("--- End ---");
    ...
    if (typeof ServiceClass !== "function") {
      throw new Error(
        `ReminderService class not exported. Got: ${JSON.stringify(Object.keys(exports))}`
      );
    }
  });
  ...
  afterAll(() => {
    log.info(
      `Generation: ${generationLatencyMs}ms | ${generationTokens} tokens | $${generationCost.toFixed(5)}`
    );
  });
  ...
  describe("create()", () => {
    ...
    test("creates reminder with all optional fields", () => {
      ...
      const r = service.create({
        title: "Weekly review",
        description: "Review the week's tasks",
        dueAt: due,
        repeat: "weekly",
        tags: ["work", "review"],
      });
      expect(r.description).toBe("Review the week's tasks");
  ...
  describe("listPending()", () => {
    test("returns only future, incomplete reminders", () => {
      ...
      expect(pending.some((r) => r.id === future.id)).toBe(true);
    });
  ...
  describe("listOverdue()", () => {
    test("returns past incomplete reminders", () => {
      ...
      const overdue = service.listOverdue();
      expect(overdue.some((r) => r.id === past.id)).toBe(true);
    });
  ...
  describe("Integration: full workflow", () => {
    test("create → list → complete → overdue flow", () => {
      ...
      // getDueWithin
      const dueSoon = service.getDueWithin(10);
      expect(dueSoon.some((r) => r.id === r2.id)).toBe(true);
