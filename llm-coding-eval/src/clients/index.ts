...
export type ModelKey = "qwen7b" | "qwen14b" | "deepseek" | "glm" | "codex" | "claude";
  ...
  switch (key) {
    ...
    case "codex":
      if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY === "your_openai_api_key_here") {
        throw new Error(
          "OPENAI_API_KEY not set. Add it to .env to run the Codex/OpenAI eval."
        );
      }
    ...
    case "claude":
      if (!process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_API_KEY === "your_anthropic_api_key_here") {
        throw new Error(
          "ANTHROPIC_API_KEY not set. Add it to .env to expose Claude through the API."
        );
      }
