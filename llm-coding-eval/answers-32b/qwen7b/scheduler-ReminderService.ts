...
class ReminderService {
  ...
  listPending(): Reminder[] {
    const now = new Date();
    return this.listAll().filter(reminder => !reminder.completed && reminder.dueAt >= now);
  }
  ...
  getDueWithin(minutes: number): Reminder[] {
    ...
    return this.listPending().filter(reminder => reminder.dueAt >= now && reminder.dueAt <= dueBy);
  }
